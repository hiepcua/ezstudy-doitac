...
if(isLogin()){
	...
	$member_packet = $member_info['packet'];
	$member_grade = $member_info['grade'];
	...
	if(is_array($rep['data']) && count($rep['data']) > 0) {
		$arr_packet = $rep['data'];
		$packet_conf = isset($arr_packet[$member_packet]['packet']) ? json_decode($arr_packet[$member_packet]['packet'],true):array();
		$packet_grade = isset($packet_conf[$member_grade]) ? $packet_conf[$member_grade]:array();
		?>
		<form method="post" action="" id="frm_action_extend">
			...
			<div class="modal-body-content">
				...
				<div class="form-group" id='packet_price'>
					<label>Thời gian:</label>
					<select name="txt_month" class="form-control txt_month" id="txt_month"> 
						<option value="">-- Chọn thời gian --</option>
						<?php
						foreach ($packet_grade as $key => $value) {
							echo '<option value="'.$key.'">Gói '.$value['name'].' - Giá '.number_format($value['money']).'đ</option>';
						}
						?>  
					</select>
			...
			<div class="modal-footer">
				...
				<span class="btn btn-danger act-delete" id="act-process" value="Ok">Gia hạn</span>
			</div>
		</form>
		<?php 
	}
...
<script>
	...
	$('#act-process').click(function(){
		...
		$.post('<?php echo ROOTHOST;?>ajaxs/account/process_extend.php',postData, function(res){
			console.log(res);
			if(res=='success'){
				$('#myModalPopup').modal('hide');
				showMess('Gia hạn dịch vụ thành công!','');
