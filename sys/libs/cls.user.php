...
class CLS_USER{
	...
	public function LOGIN($user,$pass){
		...
		if($user=='' || $pass=='')
			$flag=false;
		$sql="SELECT * FROM `sys_users` WHERE `username`='$user' AND isactive=1";
	...
	public function UpdateLogin($user,$flag){
		...
		$sql="UPDATE `sys_users` SET `lastLogin`='$value' WHERE `username`='$user'";
		return $this->objmysql->Query($sql);
	}
	public function AutoLogout($user){
		/* if(!isset($user)||$user=='')
			return;
		$sql="SELECT `lastlogin` FROM `sys_users` WHERE `username`='$user' ";
		$this->objmysql->Query($sql);
		$rows=$this->objmysql->Fetch_Assoc();
		if($rows['lastlogin']=='')
			return;
	...
	public function getList($where='',$limit=''){
		$sql="SELECT * FROM `sys_users` WHERE 1=1 ".$where.$limit;
		$this->objmysql->Query($sql);
	}
	public function getListGroup($where=''){
		$sql="SELECT * FROM `sys_guser` WHERE 1=1 ".$where;
		$this->objmysql->Query($sql);
	}
	...
	public function checkUserExists($user){
		$sql = "SELECT `username` FROM `sys_users` WHERE `username` ='$user'";
		$this->objmysql->Query($sql);
		if($this->objmysql->Num_rows()>0) return true;
		return false;
	}
	function Add_new(){
		$sql="INSERT INTO `sys_users` (`username`,`password`,`firstname`,`lastname`,`birthday`,`gender`,`address`,`phone`,`mobile`,`email`,`joindate`,`gid`,`isactive`) VALUES ";
	...
	function Update(){		 
		$sql="UPDATE `sys_users` SET 	`firstname`='".$this->FirstName."',
	...
	public function ChangePass_User($user,$newpass) {
		$sql="UPDATE `sys_users` SET `password`='".md5(sha1(trim($newpass)))."'";
		$sql.=" WHERE username='$user'"; 
		return $this->objmysql->Exec($sql);
	}
	// set active template
	function setActive($ids,$status=''){
		$sql="UPDATE `sys_users` SET `isactive`='$status' WHERE `id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `sys_users` SET `isactive`=if(`isactive`=1,0,1) WHERE `id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	function Delete($id){
		$sql="DELETE FROM `sys_users` WHERE `id` in ('$id')";
		return $this->objmysql->Query($sql);
	}
	...
	public function getGroupUser($par_id,$level=0){
		$sql="SELECT * FROM `sys_guser` WHERE par_id=$par_id AND `isactive`=1";
	...
	public function getUserInGroup($gid){
		$sql="SELECT * FROM `sys_users` WHERE `isactive`=1 AND `gid`=$gid";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		if($objdata->Num_rows()){
			echo "<ul class='menu'>";
	...
	public function getUserID($uid){
		$sql="SELECT * FROM `sys_users` WHERE `isactive`=1 AND `id`=$uid";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		if($objdata->Num_rows()){
			echo "<ul class='menu'>";
	...
	function getListCombo($parid,$thisid,$level){
		$sql="SELECT * FROM `sys_guser` WHERE `par_id`=$parid AND `isactive`=1";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		$char="";
		if($level>0){
	...
	public function getCurrentLastId() {
		$sql="SELECT max(id) as max FROM `sys_users`";
		$objdata=new CLS_MYSQL;
		$objdata->Query($sql);
	...
	public function getNameById($id){
        $objdata=new CLS_MYSQL;
        $sql="SELECT CONCAT(`firstname`,' ',`lastname`) AS 'fullname' FROM `sys_users`  WHERE isactive=1 AND `user_id` = '$id'";
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
        return $row['fullname'];
    }
	public function Permission($com='') {
		if(isset($_SESSION[MD5($_SERVER['HTTP_HOST']).'_USERLOGIN'])){
			...
			$sql="SELECT `permission` FROM sys_guser WHERE id=$gid"; 
			$this->objmysql->Query($sql);
